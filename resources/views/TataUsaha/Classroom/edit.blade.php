...
<div class="content">
    <div class="panel panel-flat">
        <form action="{{ route('classrooms.update', ['classroom' => $classroom]) }}" id="form" method="post" class="form-horizontal">
            @csrf
            @method('PUT')
            <div class="panel-body">
                ...
                <div class="form-group" @error('name') has-error @enderror">
                    <label class="control-label col-lg-2">Nama Kelas <span class="text-danger">*</span></label>
                        <div class="col-lg-10">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ $classroom->name }}"/>
                        </div>
                    </div>
                    <div class="form-group" @error('class_major') has-error @enderror">
                        ...
                        <div class="col-lg-10">
                            <div class="radio">
                                <label>
                                    <input name="class_major" value="IPA" type="radio" class="styled" @if($classroom->class_major == "IPA") checked @else @endif>IPA
                                </label>
                            </div>
                            <div class="radio">
                                <label>
                                    <input name="class_major" value="IPS" type="radio" class="styled" @if($classroom->class_major == "IPS") checked @else @endif>IPS
                                </label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group" @error('total_student') has-error @enderror">
                        ...
                        <div class="col-lg-10">
                            <input type="number" min="0" class="form-control @error('total_student') is-invalid @enderror" value="{{ $classroom->total_student }}" name="total_student"/>
                        </div>
                    </div>
